	...
	foreach ( $share_buttons as $network => $button ){
		...
		if( tie_get_option( 'share_'.$network_id.$button_position ) ){
			...
			$active_share_buttons[] = '
				<a href="'. tie_share_button_url( $button['url'], $esc ) .'" rel="external noopener" title="'. $button['text'] .'" target="_blank" class="'. $custom_button_class .' '. $button_class .'" data-raw="'. $button['url'] .'">
